...
function sharedComparableLocal(){return JSON.stringify({goal:Number(state.goal||7000),savings:(state.savings||[]).map(x=>[x.id,x.name,Number(x.amount)||0,sharedRecordDate(x),x.updatedAt||'']).sort(),expenses:(state.expenses||[]).map(x=>[x.id,x.name,x.category,Number(x.eventSlot)||0,Number(x.amount)||0,sharedRecordDate(x),x.updatedAt||'']).sort(),events:(state.events||[]).map(x=>[x.id,Number(x.slot)||1,x.name,Number(x.amount)||0,sharedRecordDate(x),x.updatedAt||'']).sort()})}
function sharedComparableRemote(remote){return JSON.stringify({goal:Number(remote?.goal||7000),savings:(remote?.savings||[]).map(x=>[x.id,x.name,Number(x.amount)||0,x.isoDate||'',x.updatedAt||'']).sort(),expenses:(remote?.expenses||[]).map(x=>[x.id,x.name,x.category,Number(x.eventSlot)||0,Number(x.amount)||0,x.isoDate||'',x.updatedAt||'']).sort(),events:(remote?.events||[]).map(x=>[x.id,Number(x.slot)||1,x.name,Number(x.amount)||0,x.isoDate||'',x.updatedAt||'']).sort()})}

function applySharedRemote(data){if(data?.workspace)sharedWorkspace={id:data.workspace.id,name:data.workspace.name,role:data.workspace.role};const remote=data?.state;if(!remote)return;const changed=sharedComparableLocal()!==sharedComparableRemote(remote);if(!changed){sharedMeta.lastSyncAt=sharedNow();sharedDirty=false;persistShared();return}sharedApplyingRemote=true;state={goal:Number(remote.goal||7000),savings:(remote.savings||[]).map(x=>({id:x.id,name:x.name,amount:Number(x.amount),date:sharedDateLabel(x.isoDate),isoDate:x.isoDate,updatedAt:x.updatedAt})),expenses:(remote.expenses||[]).map(x=>({id:x.id,name:x.name,category:x.category,eventSlot:Number(x.eventSlot)||0,amount:Number(x.amount),date:sharedDateLabel(x.isoDate),isoDate:x.isoDate,updatedAt:x.updatedAt})),events:(remote.events||[]).map(x=>({id:x.id,slot:Number(x.slot)||1,name:x.name,amount:Number(x.amount),date:sharedDateLabel(x.isoDate),isoDate:x.isoDate,updatedAt:x.updatedAt}))};sharedMeta.goalUpdatedAt=remote.goalUpdatedAt||sharedMeta.goalUpdatedAt;sharedMeta.lastSyncAt=sharedNow();sharedDirty=false;sharedDeletes=sharedDeletes.filter(x=>x.workspaceId!==sharedWorkspace?.id);try{sharedBaseSaveState()}catch{}sharedLastSnapshot=sharedSnapshot();sharedApplyingRemote=false;persistShared();render()}
...
function startSharedPolling(){clearInterval(sharedPoll);sharedPoll=setInterval(()=>{if(document.hidden||!navigator.onLine||!sharedWorkspace)return;if(sharedDirty||sharedDeletes.length)syncSharedState(false);else loadSharedState(false)},30000)}
